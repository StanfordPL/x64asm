...
void Assembler::mod_rm_sib(const M<T>& rm, const Operand& r) {
  ...
  if (!rm.contains_base()) {
    ...
    const auto sib_byte = rm.contains_index() ?
                          (((int)rm.get_scale() << 6) & 0xc0) | ((rm.get_index().first << 3) & 0x38) | 0x5 :
                          0x00 | 0x20 | 0x5;
